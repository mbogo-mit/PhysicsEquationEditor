  ...
  $(`
    <div class="editor_line row">
      <div class="line_label col m1">
        ...
        <span class="line-warning" mf="${rid}"><i class="fas fa-exclamation-triangle"></i></span>
...
function GenerateErrorMessages(id){
  if(MathFields[id] != undefined){
    ...
    let uv = GetUndefinedVariables(MathFields[id].mf.latex());
    //console.log(uv);
    if(uv.length > 0){
      if(uv.includes("d")){
        MathFields[id].message.error = {
          type: 1,
        }
      }
      else{
  ...
  if(opts.update){
    ...
    $(`#my_variables .collection-item .static-physics-equation`).each(function(){
      if($(this).attr("rid") == opts.rid){
        ...
        let html = `
          <span class="right delete-var" onclick="UpdateMyVariablesCollection({ls: '${opts.ls}',rid: '${opts.rid}',remove: true, editable: ${isVariableEditable}})"><i class="material-icons">close</i></span>
